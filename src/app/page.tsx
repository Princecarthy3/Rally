...
const games = [
  ...
  { icon: "🎲", title: "Ludo", text: "Race your tokens to victory", color: "bg-[#fff2bd]", tilt: "rotate-1" },
];
...
export default function HomePage() {
  return (
    <main className="overflow-hidden bg-[#fbfbfe]">
      ...
      <section id="games" className="bg-white px-5 py-24 md:py-32"><div className="mx-auto max-w-6xl"><div className="text-center"><p className="text-sm font-extrabold uppercase tracking-[.2em] text-violet-600">Pick your game</p><h2 className="mt-4 text-4xl font-black tracking-[-.05em] text-slate-950 md:text-6xl">Ten ways to settle it.</h2><p className="mx-auto mt-5 max-w-xl text-lg text-slate-500">Quick multiplayer games for 2–4 friends, all built to play from different places.</p></div><div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{games.map(game => <article key={game.title} className={`${game.color} ${game.tilt} group rounded-[32px] p-6 transition duration-300 hover:rotate-0 hover:-translate-y-2`}><div className="grid aspect-square place-items-center rounded-[24px] bg-white/55 text-7xl shadow-sm transition group-hover:scale-[1.03]">{game.icon}</div><h3 className="mt-6 text-xl font-extrabold tracking-tight">{game.title}</h3><p className="mt-1 text-sm text-slate-500">{game.text}</p></article>)}</div></div></section>
