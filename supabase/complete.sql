...
begin
 ...
 if p_state ? 'winnerSeat' and p_state->>'winnerSeat' is not null then winning_seats:=array[(p_state->>'winnerSeat')::int];
  ...
  select coalesce(array_agg(seat),'{}') into winning_seats from public.game_players where room_id=p_room and coalesce((p_state->'scores'->>seat::text)::int,0)=top_score;
  ...
  if distinct_choices=2 then
   ...
   select coalesce(array_agg(key::int),'{}') into winning_seats from jsonb_each_text(p_state->'choices') where value=winning_choice;
  end if;
 end if;
 ...
 for p in select gp.*,pr.display_name from public.game_players gp join public.profiles pr on pr.id=gp.player_id where gp.room_id=p_room loop
  ...
  select p_room,p.player_id,array_agg(pr.display_name order by gp.seat),p_type,current_match,result,player_score,top_score from public.game_players gp join public.profiles pr on pr.id=gp.player_id where gp.room_id=p_room and gp.player_id<>p.player_id
