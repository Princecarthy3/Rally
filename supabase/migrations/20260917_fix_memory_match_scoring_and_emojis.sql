-- Fix pair detection: JSON arrays require integer indexes, not text keys.
-- Also use actual emoji card faces and reject extra flips while a pair is revealed.
create or replace function public.start_memory_match(p_room uuid) returns void
language plpgsql security definer set search_path='' as $$
declare r public.game_rooms; n int; deck jsonb := '[]'::jsonb; symbols text[] := array['🍎','🚀','🐶','🌈','⚽','🎸','🌙','🦄']; i int;
begin
  ...
  insert into private.memory_match_decks(room_id,cards) values(p_room,deck)
  on conflict(room_id) do update set cards=excluded.cards;
  update public.game_rooms
  set status='playing',
      public_state=jsonb_build_object('turn',1,'cards',jsonb_build_array(null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null),'flipped','[]'::jsonb,'matched','[]'::jsonb,'scores','{}'::jsonb,'revealed',false,'message','Player 1 flips first!'),
...
language plpgsql security definer set search_path='' as $$
declare r public.game_rooms; me public.game_players; state jsonb; deck jsonb; cards jsonb; flipped jsonb; matched jsonb; idx int; first_idx int; second_idx int; next_seat int; score int; winner int; top_score int;
begin
  select * into r from public.game_rooms where id=p_room for update;
  ...
  cards:=coalesce(state->'cards',jsonb_build_array(null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null));
  ...
  if p_action='flip' then
    idx:=p_value::int;
    if idx<0 or idx>15
      or jsonb_array_length(flipped)>=2
      or exists(select 1 from jsonb_array_elements_text(matched) value where value::int=idx)
      or exists(select 1 from jsonb_array_elements_text(flipped) value where value::int=idx) then
    ...
    if jsonb_array_length(flipped)=2 then
      ...
      if cards->first_idx=cards->second_idx then
        ...
        state:=jsonb_set(state,'{message}',to_jsonb(('Player '||me.seat||' found a pair!')::text),true);
      else
        state:=jsonb_set(state,'{message}',to_jsonb(('Player '||me.seat||' missed. Next player goes.')::text),true);
      end if;
    end if;
    ...
    second_idx:=(flipped->>1)::int;
    if cards->first_idx=cards->second_idx then
      if jsonb_array_length(matched)=16 then
        ...
        state:=jsonb_set(state,'{message}',to_jsonb('Memory Match complete!'::text),true);
      end if;
    else
