-- Restore the action contract used by the Number Hunt client.  A later broad
-- action-RPC migration accidentally replaced this four-argument function with
-- an older three-argument implementation, so every submitted move was rejected
-- by PostgREST before it reached the game state machine.
