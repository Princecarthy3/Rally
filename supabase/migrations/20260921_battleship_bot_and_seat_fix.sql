-- Battleship: seat-keyed boards (NOT player_id) + bot auto_deploy
...
begin
  ...
  select p_room, seat, '{}'::jsonb, '[]'::jsonb from public.game_players where room_id=p_room;
  update public.game_rooms set status='playing', state_version=state_version+1,
    public_state=jsonb_build_object(
      'phase','placing','turn',1,
      'shots',jsonb_build_object('1','[]'::jsonb,'2','[]'::jsonb),
      'stats',jsonb_build_object('1',jsonb_build_object('hits',0,'misses',0,'sunk',0),'2',jsonb_build_object('hits',0,'misses',0,'sunk',0)),
...
language plpgsql security definer set search_path='' as $$
declare
  r public.game_rooms; me public.game_players; target public.game_players;
  board private.battleship_boards; target_board private.battleship_boards;
  state jsonb; v_ships jsonb; ship_id text; size int; cells jsonb;
  row_no int; col_no int; horizontal boolean; i int; cell int;
  occupied int[]; c int; ok boolean; placed_count int; both_ready boolean;
  shots jsonb; mine jsonb; hit boolean; new_hits jsonb; sunk text;
  remaining int; stats jsonb; do_lock boolean := false;
begin
  select * into r from public.game_rooms where id=p_room for update;
  ...
  if p_action='auto_deploy' then
    p_action:='randomize_fleet';
    do_lock:=true;
  end if;
  ...
  if p_action='randomize_fleet' then
    ...
    v_ships:='{}'::jsonb; occupied:=array[]::int[];
    foreach ship_id in array array['carrier','battleship','cruiser','submarine','destroyer'] loop
      ...
      for i in 1..120 loop
        ...
        row_no:=floor(random()*8)::int; col_no:=floor(random()*8)::int;
        ...
        if ok then
          for c in 0..jsonb_array_length(cells)-1 loop occupied:=occupied||(cells->>c)::int; end loop;
    ...
    if do_lock then p_action:='lock_fleet'; end if;
  end if;

  if p_action='place_ship' then
    if coalesce(state->>'phase','placing')<>'placing' then raise exception 'Fleet is locked'; end if;
    if coalesce((state->'placements'->>me.seat::text)::boolean,false) then raise exception 'Fleet already locked'; end if;
    ...
    new_hits:=coalesce(target_board.hits,'[]'::jsonb); sunk:=null;
    if hit then
      ...
      select e.key into sunk from jsonb_each(target_board.ships) e
      where not exists(select 1 from jsonb_array_elements_text(e.value->'cells') x where not (new_hits @> jsonb_build_array(x.value::int)))
        and exists(select 1 from jsonb_array_elements_text(e.value->'cells') x where x.value::int=cell)
      limit 1;
    end if;
    ...
    select count(*) into remaining from jsonb_each(coalesce(target_board.ships,'{}'::jsonb)) e
    where exists(select 1 from jsonb_array_elements_text(e.value->'cells') x
    ...
    if remaining=0 then
      ...
      state:=jsonb_set(state,'{message}',to_jsonb(
        case when sunk is not null then ('Sunk the '||initcap(sunk)||'! Player '||target.seat||' to fire.')
             when hit then ('Hit! Player '||target.seat||' to fire.')
             else ('Miss. Player '||target.seat||' to fire.') end),true);
    end if;
  elsif p_action='randomize_fleet' then
    null; -- handled above; already updated state
  else
