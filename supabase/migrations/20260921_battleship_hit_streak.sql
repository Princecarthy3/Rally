...
-- Keep the turn after a hit; only pass turn on a miss (or game over).
...
begin
  ...
  if p_action='place_ship' then
    ...
    if both_ready then
      ...
      state:=jsonb_set(state,'{message}','"Battle stations! Hit keeps your turn — miss passes it."'::jsonb,true);
    else
    ...
    if remaining=0 then
      ...
      -- HIT STREAK: keep the turn
      state:=jsonb_set(state,'{turn}',to_jsonb(me.seat),true);
      state:=jsonb_set(state,'{message}',to_jsonb(
        case when sunk is not null
          then ('Sunk the '||initcap(sunk)||'! Fire again!')
          else 'Hit! Fire again!'
        end),true);
    else
      state:=jsonb_set(state,'{turn}',to_jsonb(target.seat),true);
      state:=jsonb_set(state,'{message}',to_jsonb(('Miss. Player '||target.seat||' to fire.')::text),true);
    end if;
