-- Fix bot actor seat matching (multi-bot UUIDs) + reveal word when round times out
...
begin
  ...
  -- Bot moves: match any Rally bot UUID for the given seat (…1112, …1113, etc.)
  ...
  if p_action='select_word' then
    ...
    if seconds_left=0 or resolved >= n-1 then
      -- Always surface the secret word when the round closes without full success path
      if p_action='time_expired' or resolved < n-1 then
        revealed := coalesce(nullif(trim(state->>'revealedWord'),''), nullif(trim(state->>'wordSelected'),''), '');
        if revealed <> '' and revealed <> 'null' then
          state:=jsonb_set(state,'{revealedWord}',to_jsonb(revealed),true);
          if p_action='time_expired' then
            state:=jsonb_set(state,'{message}',to_jsonb(('Time is up! The word was: '||revealed)::text),true);
          end if;
      ...
      if current_round >= n*2 then
        ...
        if count_tied=1 then
          ...
          state:=jsonb_set(state,'{message}',to_jsonb(('Tiebreaker. Last word was shown. Next drawer: Player '||next_seat||'.')::text),true);
        end if;
      else
        state:=jsonb_set(state,'{round}',to_jsonb(current_round+1),true);
        state:=jsonb_set(state,'{drawerSeat}',to_jsonb(next_seat),true);
        ...
        state:=jsonb_set(state,'{roundStartedAt}','null'::jsonb,true);
        if p_action='time_expired' then
          state:=jsonb_set(state,'{message}',to_jsonb(('Time up! The word was: '||coalesce(revealed, '?')||'. Next drawer: Player '||next_seat||'.')::text),true);
        else
          state:=jsonb_set(state,'{message}',to_jsonb(('Round over. Next drawer: Player '||next_seat||'.')::text),true);
        end if;
      end if;
    end if;
  else
