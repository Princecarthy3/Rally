  ...
  -- =========================================================================
    ...
    if jsonb_array_length(v_hand) = 0 then
      ...
      if v_new_score >= 500 then
        ...
        v_card_counts := jsonb_set(v_card_counts, array[me.seat::text], '0'::jsonb, true);
      else
        -- Auto-start next round: reshuffle, deal 7 each, new top card
        declare
          v_full jsonb;
          v_shuf jsonb;
          v_seat_i int;
          ...
          v_round int;
        begin
          select array_agg(seat order by seat) into v_seats from public.game_players where room_id = p_room;
          v_full := public.generate_uno_deck();
          select jsonb_agg(elem order by random()) into v_shuf from jsonb_array_elements(v_full) elem;
          v_hands := '{}'::jsonb;
          v_card_counts := '{}'::jsonb;
          foreach v_seat_i in array v_seats loop
            v_new_hand := '[]'::jsonb;
            for i in 1..7 loop
              v_new_hand := v_new_hand || jsonb_build_array(v_shuf->0);
              select jsonb_agg(elem) into v_shuf from (
                select elem, row_number() over () as idx from jsonb_array_elements(v_shuf) elem
              ) t where idx > 1;
            end loop;
            v_hands := jsonb_set(v_hands, array[v_seat_i::text], v_new_hand, true);
            v_card_counts := jsonb_set(v_card_counts, array[v_seat_i::text], '7'::jsonb, true);
          end loop;

          while (v_shuf->v_idx->>'value') = 'wild_draw4' loop
            v_idx := v_idx + 1;
          end loop;
          v_top := v_shuf->v_idx;
          select jsonb_agg(elem) into v_shuf from (
            select elem, row_number() over () as row_n from jsonb_array_elements(v_shuf) elem
          ) s where row_n <> (v_idx + 1);

          v_deck := v_shuf;
          v_top_card := v_top;
          if (v_top->>'color') = 'wild' then
            v_active_color := 'red';
          else
            v_active_color := v_top->>'color';
          end if;
          v_dir := 1;
          v_turn := me.seat; -- round winner starts next round
          v_uno_called := '{}'::jsonb;
          v_uno_vulnerable := null;
          v_pending := null;
          v_round := coalesce((state->>'round')::int, 1) + 1;
          state := jsonb_set(state, '{round}', to_jsonb(v_round), true);
          state := jsonb_set(state, '{roundWinnerSeat}', to_jsonb(me.seat), true);
          state := jsonb_set(state, '{discardCount}', '1'::jsonb, true);
          state := jsonb_set(state, '{drawCount}', to_jsonb(jsonb_array_length(v_shuf)), true);
          v_msg := 'Player ' || me.seat || ' won the round (+' || v_round_pts || ' pts)! Score ' || v_new_score || '/500. Round ' || v_round || ' — new hands dealt. Player ' || me.seat || ' starts.';
        end;
      end if;
    end if;
    ...
    state := jsonb_set(state, '{unoCalled}', coalesce(v_uno_called, '{}'::jsonb), true);
