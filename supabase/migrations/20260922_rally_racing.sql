...
alter table public.game_rooms add constraint game_rooms_game_type_check
  check (game_type in (
    ...
    'ping_pong','tic_tac_toe','connect_four','dots_boxes','skribbl','ludo','racing'
  ));
...
begin
  ...
  if definition is not null then
    definition := replace(definition, '''ludo''', '''ludo'',''racing''');
    execute definition;
  end if;
end $$;
...
begin
  select * into r from public.game_rooms where id=p_room for update;
  if r.status <> 'playing' or r.game_type <> 'racing' then raise exception 'Racing is not active'; end if;
  ...
  if p_action = 'start_race' then
    ...
    if not exists(select 1 from jsonb_array_elements(results) elem where (elem->>'seat')::int = me.seat) then
      ...
      -- If all players finished, complete room
